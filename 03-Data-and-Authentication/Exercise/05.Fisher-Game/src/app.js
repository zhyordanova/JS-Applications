...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('catches').addEventListener('click', onUpdateOrDeleteClick)
});
...
//     const fishCatch = await fetch('http://localhost:3030/data/catches/' + id);
//     return fishCatch;
// }
...
async function onUpdateCatch(id, {angler, weight, species, location, bait, captureTime}) {
    const res = await fetch('http://localhost:3030/data/catches/' + id, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
...
async function onUpdateOrDeleteClick(event) {
    if (event.target.tagName == 'BUTTON' && event.target.className == 'update') {
        onUpdateCatch(event.target);

    } else if (event.target.tagName == 'BUTTON' && event.target.className == 'delete') {
        onDeleteCatch(event.target);
    }
}

async function onDeleteCatch(button) {
    const id = button.dataset.id;
    await deleteCatch(id),
    loadData();
}

async function onUpdateCatch(button) {
    const id = button.dataset.id;
    const [angler, weight, species, location, bait, captureTime] = button.parentNode.querySelectorAll('input')
    await onUpdateCatch(id, {angler, weight, species, location, bait, captureTime})
