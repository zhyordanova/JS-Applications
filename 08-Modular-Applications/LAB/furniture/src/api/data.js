...
export async function getAll(page, search) {
    let url_1 = endpoints.all + (page - 1) * pageSize;
    let url_2 = endpoints.count;

    if (search) {
        url_1 += '&where=' + encodeURIComponent(`make LIKE "${search}"`);
        url_2 += '&where=' + encodeURIComponent(`make LIKE "${search}"`);
    }
